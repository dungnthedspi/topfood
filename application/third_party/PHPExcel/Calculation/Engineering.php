...
class PHPExcel_Calculation_Engineering {
	...
	public static function IMARGUMENT($complexNumber) {
		...
		if ($parsedComplex['real'] == 0.0) {
			if ($parsedComplex['imaginary'] == 0.0) {
				return 0.0;
			} else if ($parsedComplex['imaginary'] < 0.0) {
				return M_PI / -2;
			} else {
				return M_PI / 2;
			}
		} else if ($parsedComplex['real'] > 0.0) {
	...
	public static function IMLOG10($complexNumber) {
		...
		if (($parsedComplex['real'] == 0.0) && ($parsedComplex['imaginary'] == 0.0)) {
			return PHPExcel_Calculation_Functions::NaN();
		} else if (($parsedComplex['real'] > 0.0) && ($parsedComplex['imaginary'] == 0.0)) {
	...
	public static function IMSUB($complexNumber1, $complexNumber2) {
		...
		if ((($parsedComplex1['suffix'] != '') && ($parsedComplex2['suffix'] != '')) &&
			...
			return PHPExcel_Calculation_Functions::NaN();
		} else if (($parsedComplex1['suffix'] == '') && ($parsedComplex2['suffix'] != '')) {
			$parsedComplex1['suffix'] = $parsedComplex2['suffix'];
		}
	...
	public static function IMSUM() {
		...
		foreach ($aArgs as $arg) {
			...
			} else if (($parsedComplex['suffix'] != '') && ($activeSuffix != $parsedComplex['suffix'])) {
				return PHPExcel_Calculation_Functions::VALUE();
			}
	...
	public static function CONVERTUOM($value, $fromUOM, $toUOM) {
		...
		if (($fromUOM == $toUOM) && ($fromMultiplier == $toMultiplier)) {
			...
			} else if ((($fromUOM == 'K') || ($fromUOM == 'kel')) &&
				(($toUOM == 'K') || ($toUOM == 'kel'))
			) {
				return $value;
			} else if ((($fromUOM == 'C') || ($fromUOM == 'cel')) &&
				(($toUOM == 'C') || ($toUOM == 'cel'))
			) {
				return $value;
			}
