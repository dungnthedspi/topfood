...
class PHPExcel_Calculation {
	...
	private function showValue($value) {
		if ($this->_debugLog->getWriteDebugLog()) {
			...
			} else if (is_string($value) && (trim($value, '"') == $value)) {
				return '"' . $value . '"';
...
//echo 'Element is a Positive number, not Plus operator', PHP_EOL;
			...
			} else if ((($opCharacter == '~') || ($opCharacter == '|')) && (!$isOperandOrFunction)) {    //    We have to explicitly deny a tilde or pipe, because they are legal
				return $this->raiseFormulaError("Formula Error: Illegal character '~'");                //        on the stack but not in the input expression

			} else if ((isset(self::$operators[$opCharacter]) or $isOperandOrFunction) && $expectingOperator) {    //    Are we putting an operator on the stack?
//echo 'Element with value '.$opCharacter.' is an Operator', PHP_EOL;
			...
			} else if ($opCharacter == ')' && $expectingOperator) {            //    Are we expecting to close a parenthesis?
...
//echo 'Built-in function '.$functionName, PHP_EOL;
					...
					} else if (isset(self::$PHPExcelFunctions[$functionName])) {
...
//echo PHP_EOL;
			...
			} else if ($opCharacter == ',') {            //    Is this the separator for function arguments?
...
//                echo 'Element is an Opening Bracket<br />';
			...
			} else if ($isOperandOrFunction && !$expectingOperator) {    // do we now have a function/variable/number?
...
//                    echo 'Element is a Variable, Constant, String, Number or Boolean<br />';
					...
					if ($testPrevOp['value'] == ':') {
						...
						} else if ((ctype_alpha($startRowColRef)) && (ctype_alpha($val)) &&
...
//                        echo 'Element '.$localeConstant.' is an Excel Constant<br />';
			...
			} else if ($opCharacter == '$') {    // absolute row or column range
				++$index;
			} else if ($opCharacter == ')') {    // miscellaneous error checking
				if ($expectingOperand) {
					$output[] = array('type' => 'NULL Value', 'value' => self::$excelConstants['NULL'], 'reference' => null);
					$expectingOperand = false;
					$expectingOperator = true;
			...
			} else if (isset(self::$operators[$opCharacter]) && !$expectingOperator) {
...
//                    echo 'Token is a PHPExcel constant: '.$excelConstant.'<br />';
				...
				} else if ((is_numeric($token)) || ($token === null) || (is_bool($token)) || ($token == '') || ($token{0} == '"') || ($token{0} == '#')) {
...
//                echo 'Named Range '.$pRange.' (';
				...
				} else if (ctype_digit($splitRange[0][0])) {
